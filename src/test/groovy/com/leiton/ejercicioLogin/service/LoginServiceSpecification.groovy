...
import com.leiton.ejercicioLogin.dto.PhoneRequest
...
class LoginServiceSpecification extends Specification {
    ...
    def "sign-up should validate correct request"(){
        when:
            loginService.signUp(new SignUpRequest("name", "test1@mail.com", "a2asfGfdfdf4", [new PhoneRequest(123L, 456, "AR")]))
    ...
    def "sign-up should throw exception on incorrect email"(){
        when:
            loginService.signUp(new SignUpRequest("name", "test2@mail", "a2asfGfdfdf4", [new PhoneRequest(123L, 456, "AR")]))
    ...
    def "sign-up should throw exception on incorrect password"(){
        when:
            loginService.signUp(new SignUpRequest("name", "test3@mail.com", password, [new PhoneRequest(123L, 456, "AR")]))
    ...
    def "sign-up shouldn't let register an already registered user"(){
        when:
            loginService.signUp(new SignUpRequest("name", "test6@mail.com", "a2asfGfdfdf4", [new PhoneRequest(123L, 456, "AR")]))
            loginService.signUp(new SignUpRequest("otherName", "test6@mail.com", "a2asfGfdfdf4", [new PhoneRequest(123L, 456, "AR")]))
    ...
    def "login should retrieve correct user" (){
        when:
            def response = loginService.signUp(new SignUpRequest("name", "test7@mail.com", "a2asfGfdfdf4", [new PhoneRequest(123L, 456, "AR")]))
    ...
    def "login should change token when logged in" () {
        when:
            def signUpResponse = loginService.signUp(new SignUpRequest("name", "test8@mail.com", "a2asfGfdfdf4", [new PhoneRequest(123L, 456, "AR")]))
